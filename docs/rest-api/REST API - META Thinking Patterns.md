...
> **Code walkthrough:** This Resource-Oriented Thinking Mental Model example demonstrates a key concept in practice using SQL. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
**[JUNIOR] Q1 - [CONCEPTUAL] "How do you model a domain action that doesn't fit CRUD?"**
...
**[JUNIOR] Q2 - [CONCEPTUAL] "How would you model a REST API for a state machine (e.g., order lifecycle)?"**
...
**[JUNIOR] Q3 - [CONCEPTUAL] "What problems arise when procedures are mixed with resources in an API?"**
...
**[MID] Q4 - [CONCEPTUAL] "How does resource-oriented thinking apply to microservices boundaries?"**
...
**[MID] Q5 - [ARCHITECTURE] "What interview answer do you give when asked to design a REST API for a complex domain?"**
...
**[SENIOR] Q6 - [TRADE-OFF] "When is RPC-style design (POST /sendEmail,
POST /cancelOrder) better than resource-oriented REST?"**
> "RPC-style APIs model the domain as operations rather than resources. They
are appropriate when: (1) The operation has no natural resource representation:
`POST /evaluateRiskScore` is a computation, not a CRUD operation on a resource.
There's no 'RiskScore' resource to GET, PUT, or DELETE. (2) The operation is
complex enough to warrant its own semantic: `POST /sendEmail` with 15 parameters
is clearer than `POST /emails` with status transitions. (3) Internal service APIs:
when the API is consumed only by internal services that share the team's
domain model, RPC-style APIs are often simpler and faster to implement
(gRPC/Thrift). REST's uniform interface has overhead that's only valuable for
external APIs. (4) Event-driven semantics: `POST /orders/{id}/cancellations`
vs `PATCH /orders/{id}` with `status: cancelled`. The former models cancellation
as a first-class event with its own resource (history, metadata, reason).
The rule: REST for external APIs with unknown clients (uniform interface benefits
are worth the overhead). RPC for internal service-to-service calls (efficiency
and type safety via gRPC). Hybrid for complex domains (resource-oriented core +
action endpoints for operations that don't fit CRUD)."

*What separates good from great:* "Framing it as internal vs external API
boundary. RPC and REST serve different consumers. Using gRPC internally and
REST externally is a legitimate production architecture, not inconsistency."

---

**[SENIOR] Q7 - [BEHAVIORAL] "Describe a time you had to refactor a
poorly designed REST API without breaking existing clients."**
> "Structure: situation, constraint, approach, outcome.
>
> Example: 'Our payments API had grown organically with inconsistent naming:
> `POST /makePayment`, `GET /getPaymentStatus/{id}`, `POST /refundTransaction`.
> We needed to add a new version but couldn't break the 3 mobile apps still
> on the old endpoints (they had 6-month release cycles).
>
> Approach: defined the new resource-oriented API design alongside the old
> (`POST /payments`, `GET /payments/{id}`, `POST /payments/{id}/refunds`).
> Kept old endpoints running on `v1`, new endpoints on `v2`. Added a routing
> layer that translated v1 calls to v2 internally. This meant v2 was the
> single source of truth - v1 was just an adapter.
>
> Over 6 months: all mobile app versions moved to v2 through normal
> release cycles. Deprecated v1 with `Sunset` headers 3 months before
> shutdown. Logged v1 usage to confirm zero traffic before removing.
>
> Key decisions: (1) Always route both versions to a single implementation.
> Dual implementations diverge. (2) Sunset headers communicate the shutdown
> date in the HTTP contract - no surprise removals. (3) Monitor actual
> usage, not developer promises, before deprecation."

*What separates good from great:* "The routing layer that translates v1
to v2 (adapter pattern) rather than dual implementations. This keeps the
business logic in one place. Candidates who describe maintaining two
complete implementations show they haven't done this at scale."
...
> **Code walkthrough:** This Statelessness as a Scalability Principle example demonstrates a key concept in practice using authentication. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
**[JUNIOR] Q1 - [CONCEPTUAL] "Why does statelessness enable horizontal scaling?"**
...
**[JUNIOR] Q2 - [TRADE-OFF] "What are the trade-offs of statelessness and when would you accept some state?"**
...
**[JUNIOR] Q3 - [ARCHITECTURE] "How does statelessness interact with JWT token design?"**
...
**[MID] Q4 - [DEBUGGING] "How do you debug a statelessness violation in a distributed system?"**
...
**[MID] Q5 - [ARCHITECTURE] "How does statelessness apply to idempotency in API design?"**
...
**[SENIOR] Q6 - [MECHANISM] "A client sends a POST request that times out.
The server processed the request but the ack was lost. How do you design
for this?"**
> "The lost ack problem: network partition after server commits but before
the client receives the response. Without protection: client retries -> duplicate
operation (double charge, double order). With idempotency keys: client generates
a UUID before the call. Sends it as `Idempotency-Key: {uuid}`. Server:
(1) Check Redis for this key. If found: return the stored result immediately.
(2) Execute the operation. (3) Store the result in Redis with the key, TTL 24h.
(4) Return the result. On timeout + retry: server finds the key in Redis and
returns the original result. No duplicate. Implementation details: use Redis
`SET NX EX` (set if not exists with expiry) to atomically check and reserve
the key. Between reservation and result storage: if the server crashes, the
client retries, finds the key reserved but no result. Handle this with a
`PROCESSING` status: if key exists but result is null, return 202 Accepted
and ask client to poll. When the operation completes: update to final result.
The expiry window: 24 hours balances safety (long enough for client retries)
and cache size (old keys are evicted)."

*What separates good from great:* "The race condition between key reservation
and result storage (server crash window). Candidates who describe idempotency
but miss this race condition have not implemented it in production."

---

**[SENIOR] Q7 - [DEBUGGING] "Your stateless API is experiencing 'sticky'
behavior - some users always hit one server and others hit another, and
outcomes differ. How do you diagnose?"**
> "A stateless API should be symmetric: any server should return identical
results for the same request. Asymmetry indicates state is leaking into
server-local storage. Diagnosis steps: (1) Check server-local caches:
an in-memory cache on the server holds state from previous requests.
If cache is warm on Server A but cold on Server B, results differ.
Fix: use a distributed cache (Redis) or invalidate server-local caches
on deployment. (2) Check instance-local config: feature flags or config
loaded at startup differ between instances. If Server A has a new feature
flag enabled and Server B doesn't (different deploy time or failed reload):
behavior differs. Fix: centralized config service with push updates.
(3) Check JVM/process state: static variables holding state. A static Map
that's populated on first request creates instance-local state. Fix: replace
with distributed state. (4) Check network routing: a misconfigured load
balancer doing session affinity (sticky sessions) is routing the same client
to the same server. The API might be truly stateless but the network is
behaving statelessly. Fix: remove session affinity from LB config.
Diagnosis technique: add server ID to response headers (`X-Served-By:
server-3`). Reproduce the different outcomes, check which server ID
correlates with each outcome."

*What separates good from great:* "The `X-Served-By` debugging header as
a diagnostic tool. This is a production technique for identifying which
server is misbehaving. Combined with distributed tracing, it pinpoints
the exact server and request path."
...
> **Code walkthrough:** This API Contract First Design Mindset example demonstrates a key concept in practice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
**[JUNIOR] Q1 - [HANDS-ON] "How do you implement Contract First in a team environment?"**
...
**[JUNIOR] Q2 - [CONCEPTUAL] "What is a breaking API change and how do you prevent it?"**
...
**[JUNIOR] Q3 - [CONCEPTUAL] "How does Contract First enable parallel development?"**
...
**[MID] Q4 - [CONCEPTUAL] "What is consumer-driven contract testing and how does it differ from API contract testing?"**
...
**[MID] Q5 - [CONCEPTUAL] "How do you version an API that has existing consumers?"**
...
**[MID] Q6 - [CONCEPTUAL] "What role does the OpenAPI spec play in API governance?"**
...
**[SENIOR] Q7 - [ARCHITECTURE] "How does API Contract First design relate to Domain-Driven Design?"**
