...
> **Code walkthrough:** This Regions and Availability Zones example demonstrates a key concept in practice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
> **Code walkthrough:** The Terraform example uses a dataice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
> **Code walkthrough:** This Check ALB request count per AZ: example demonstrates shell script pattern. **KEY MECHANISM:** the shell executes commands sequentially; pipes pass stdout of one command to stdin of the next. **WHY IT MATTERS:** unquoted variables with spaces cause word splitting - IFS splits the value into multiple arguments. **TAKEAWAY: always double-quote variables: "$VAR"; use [[ ]] instead of [ ] for safer conditionals.**
...
**[JUNIOR] Q1 - [TRADE-OFF] What is the difference between a Region and an Availability Zone? When does a failure in one AZ affect another?**
...
**[JUNIOR] Q2 - [DESIGN] Why does AWS charge for cross-AZ data transfer? What architecture patterns minimize this cost?**
...
**[JUNIOR] Q3 - [TRADE-OFF] What is the multi-AZ vs single-AZ trade-off for RDS? When would you choose single-AZ?**
...
**[MID] Q4 - [MECHANISM] A company's application runs in us-east-1 only. The CTO wants a disaster recovery strategy. What options exist and what drives the cost?**
...
**[MID] Q5 - [DEBUGGING] CloudWatch shows high latency for your multi-AZ web application. How do you determine if an AZ is experiencing degraded performance?****
...
**[MID] Q6 - [TRADE-OFF] For a global application, when should you deploy to multiple AWS Regions vs multiple AZs within one Region?****
...
**[SENIOR] Q7 - [MECHANISM] How does the concept of Availability Zones relate to Kubernetes node placement and fault tolerance?**
...
> **Code walkthrough:** This VPC and Cloud Networking Fundamentals example demonstrates a key concept in practice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
> **Code walkthrough:** The two route tables define theice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
> **Code walkthrough:** This (it must be in public subnet to work) example demonstrates shell script pattern. **KEY MECHANISM:** the shell executes commands sequentially; pipes pass stdout of one command to stdin of the next. **WHY IT MATTERS:** unquoted variables with spaces cause word splitting - IFS splits the value into multiple arguments. **TAKEAWAY: always double-quote variables: "$VAR"; use [[ ]] instead of [ ] for safer conditionals.**
...
> **Code walkthrough:** This Check target SG allows the source: example demonstrates shell script pattern. **KEY MECHANISM:** the shell executes commands sequentially; pipes pass stdout of one command to stdin of the next. **WHY IT MATTERS:** unquoted variables with spaces cause word splitting - IFS splits the value into multiple arguments. **TAKEAWAY: always double-quote variables: "$VAR"; use [[ ]] instead of [ ] for safer conditionals.**
...
**[JUNIOR] Q1 - [MECHANISM] Explain the purpose of a VPC and how it differs from a physical data center network.**
...
**[JUNIOR] Q2 - [TRADE-OFF] What is the difference between public and private subnets? How does a NAT Gateway work?**
...
**[JUNIOR] Q3 - [TRADE-OFF] Explain Security Groups vs Network ACLs. When does each apply?**
...
**[MID] Q4 - [DESIGN] Design a VPC for a three-tier web application (web, app, database). What subnets, routing, and security groups would you create?**
...
**[MID] Q5 - [DEBUGGING] An EC2 instance in a private subnet cannot make outbound HTTPS calls to an external API. How do you debug?****
...
**[MID] Q6 - [TRADE-OFF] When would you use VPC Peering vs AWS Transit Gateway for connecting multiple VPCs?****
...
**[SENIOR] Q7 - [MECHANISM] What is VPC Flow Logs and how do you use them for security analysis?**
...
> **Code walkthrough:** This Cloud Storage Types example demonstrates a key concept in practice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
...
> **Code walkthrough:** This Cannot be overridden per-bucket example demonstrates shell script pattern. **KEY MECHANISM:** the shell executes commands sequentially; pipes pass stdout of one command to stdin of the next. **WHY IT MATTERS:** unquoted variables with spaces cause word splitting - IFS splits the value into multiple arguments. **TAKEAWAY: always double-quote variables: "$VAR"; use [[ ]] instead of [ ] for safer conditionals.**
...
**[JUNIOR] Q1 - [TRADE-OFF] Explain the difference between object storage, block storage, and file storage. For each, give a concrete use case where it is the best choice.**
...
**[JUNIOR] Q2 - [MECHANISM] Why can't you simply store all data in S3 and avoid managing block storage?**
...
**[JUNIOR] Q3 - [TRADE-OFF] S3 has multiple storage classes. When would you use Glacier vs S3 Standard? What are the retrieval implications?**
...
**[MID] Q4 - [MECHANISM] What is EBS and how does it differ from instance store?**
...
**[MID] Q5 - [DEBUGGING] An application reads an S3 object immediately after writing it, but gets 404 (not found). Why, and how do you fix it?****
...
**[MID] Q6 - [TRADE-OFF] Compare EFS (Elastic File System) to S3 for storing machine learning training data. When is each appropriate?****
...
**[SENIOR] Q7 - [MECHANISM] How does S3 object versioning work? What are the cost implications of enabling it?**
