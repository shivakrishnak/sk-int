...
**Misconception 1: Virtualization is only needed for very large lists (1000+ items).**

Virtualization (windowing) improves performance for ANY list where the DOM node count significantly impacts layout and paint performance. 200 complex card components can cause perceptible jank during scroll on mid-range mobile devices. The threshold depends on component complexity (simple `<li>` vs complex cards with images and animations). Profile on the lowest-spec target device; if scroll frame rate drops below 60fps, virtualization is warranted.

**Misconception 2: React DevTools Profiler shows which components are slow.**

React Profiler shows render time within React. It does NOT show network time, main thread blocking from third-party scripts, or non-React DOM operations. A component that renders in 0.5ms in Profiler can still feel slow if it renders 300 times per scroll event or triggers expensive browser layout/paint operations. Use Chrome Performance tab for full-stack performance analysis including non-React causes.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: Performance optimization breaks functionality by over-memoizing.**

Symptom: component does not update when expected after aggressive memoization; state or prop changes are silently ignored; bugs introduced by optimization. Root cause: incorrect dependency array in useMemo/useCallback or stale comparison in React.memo's custom comparison function. Diagnosis: temporarily remove the optimization; verify the functionality works; then carefully re-add with correct dependencies. Fix: always test functionality after adding memoization; never use `React.memo` with a custom comparison function unless you are certain about equality semantics.

**Failure Mode 2: Bundle size optimization degrades user experience on repeat visits.**

Symptom: aggressive code splitting results in many small network requests on navigation; cached visit performance is worse than a single large bundle. Root cause: over-splitting creates too many small chunks that each require a separate network request; HTTP/2 multiplexing helps but has limits. Fix: balance splitting granularity - route-level splitting is almost always beneficial; component-level splitting should be reserved for genuinely large components or rarely-visited features.
...
**Misconception 1: React.memo prevents all re-renders of a component.**

React.memo skips re-renders when props are shallowly equal. It does NOT prevent re-renders caused by: state changes within the component, context value changes (all context consumers re-render), or calls to `forceUpdate()`. Wrapping a component that uses `useContext` with React.memo only prevents re-renders from parent prop changes - the component still re-renders whenever the relevant context updates.

**Misconception 2: React.memo is free - applying it to all components is always beneficial.**

React.memo has a cost: comparing previous and current props via shallow equality on every render. For components that almost always re-render when their parent re-renders, this cost is pure overhead. React.memo is beneficial when a component's renders are genuinely expensive AND the component frequently skips rendering because props are stable. Profile before memoizing; do not apply React.memo preventively.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: React.memo with object props never skips renders because objects are always newly created.**

Symptom: React DevTools profiler shows memoized component re-rendering on every parent render; React.memo appears to have no effect. Root cause: parent passes `style={{ color: 'red' }}` or `config={{ key: 'value' }}` inline - new object created on every render, shallow equality check fails because the reference is different even if the content is the same. Fix: memoize object props with useMemo in the parent, or move constant objects outside the component render.

**Failure Mode 2: Custom comparison function in React.memo causes stale renders.**

Symptom: component renders with outdated data; state that should trigger a re-render is silently ignored. Root cause: custom comparison `React.memo(Component, (prev, next) => prev.id === next.id)` only compares id; other prop changes are ignored. Fix: custom comparison functions should return `false` (re-render) when ANY relevant prop changes; when in doubt, use the default shallow equality.
