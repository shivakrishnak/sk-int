...
| # | Keyword | Weight |
|---|---|---|
| 1 | [React Architecture at Scale and Micro-frontends](#react-architecture-at-scale-and-micro-frontends) | architect |
| 2 | [React Security Vulnerabilities and Mitigations](#react-security-vulnerabilities-and-mitigations) | expert |
| 3 | [React Server Components and Suspense](#react-server-components-and-suspense) | expert |
| 4 | [React Fiber Reconciler Internals](#react-fiber-reconciler-internals) | expert |
| 5 | [React Testing Library Patterns](#react-testing-library-patterns) | intermediate |
| 6 | [React Anti-patterns](#react-anti-patterns) | intermediate |
| 7 | [Controlled vs Uncontrolled Components](#controlled-vs-uncontrolled-components) | intermediate |
| 8 | [React Query and Server-State Management](#react-query-and-server-state-management) | intermediate |
| 9 | [React Performance Optimization Techniques](#react-performance-optimization-techniques) | intermediate |
| 10 | [React.memo and Re-render Prevention](#reactmemo-and-re-render-prevention) | intermediate |
| 11 | [React Router and Client-side Routing](#react-router-and-client-side-routing) | working |
| 12 | [Dynamic Routing and Code Splitting](#dynamic-routing-and-code-splitting) | working |
| 13 | [Higher-Order Components](#higher-order-components) | working |
| 14 | [Render Props and Compound Components](#render-props-and-compound-components) | working |
...
**Misconception 1: Micro-frontends solve team scaling problems automatically.**

Micro-frontends enable independent deployment and team ownership of UI sections, but they introduce: bundle duplication (each MFE may include its own React copy), shared state complexity (cross-MFE communication requires event buses or shared state stores), consistent UX challenges (each team building their own component library), and significantly higher infrastructure complexity (Module Federation configuration, routing orchestration, versioning contracts). Micro-frontends trade monolith coupling for distributed systems complexity - they are only appropriate when team independence justifies this overhead.

**Misconception 2: Feature flags are the same as micro-frontends for team isolation.**

Feature flags allow toggling features at runtime within a single deployment. Micro-frontends allow separate teams to deploy their UI sections independently. A feature flag within a monolith still requires coordination on the single deployment pipeline. Micro-frontends give each team a fully independent CI/CD pipeline, runtime environment, and deployment schedule. They solve different problems: feature flags for gradual rollout; micro-frontends for organizational autonomy.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: Multiple React instances in Module Federation causes hook errors.**

Symptom: `Invalid hook call` error when a micro-frontend uses hooks; occurs only in the integrated shell, not in isolation. Root cause: shell and micro-frontend each load their own copy of React; hooks must be called within the React instance that owns the fiber tree. Fix: configure Module Federation to share React as a singleton: `shared: { react: { singleton: true, requiredVersion: '^18' } }` in both shell and MFE webpack configs.

**Failure Mode 2: Cross-MFE navigation breaks browser history.**

Symptom: browser back button does not work correctly after navigating between micro-frontends; history stack is inconsistent; direct URL access to a micro-frontend route fails. Root cause: multiple router instances each managing their own history; or the orchestration layer not propagating navigation events correctly. Fix: use a single routing authority in the shell application; micro-frontends receive their current path as a prop or via a shared routing service and do not manage top-level browser history themselves.
...
> **Code walkthrough:** This React Fiber Reconciler Internals example demonstratice. **KEY MECHANISM:** the runtime executes these instructions in sequence with specific memory and execution semantics. **WHY IT MATTERS:** misapplying this pattern causes subtle bugs that only manifest under production load. **TAKEAWAY: understand the execution model before using this pattern in production code.**
