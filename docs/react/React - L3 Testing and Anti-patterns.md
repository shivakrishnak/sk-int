---
layout: default
title: "React - L3 Testing and Anti-patterns"
parent: "React"
nav_order: 10
...
| # | Keyword | Weight |
|---|---|---|
| 1 | [React Testing Library Patterns](#react-testing-library-patterns) | intermediate |
| 2 | [React Anti-patterns](#react-anti-patterns) | intermediate |
| 3 | [Controlled vs Uncontrolled Components](#controlled-vs-uncontrolled-components) | intermediate |
| 4 | [React Query and Server-State Management](#react-query-and-server-state-management) | intermediate |
| 5 | [React Performance Optimization Techniques](#react-performance-optimization-techniques) | intermediate |
| 6 | [React.memo and Re-render Prevention](#reactmemo-and-re-render-prevention) | intermediate |
| 7 | [React Router and Client-side Routing](#react-router-and-client-side-routing) | working |
| 8 | [Dynamic Routing and Code Splitting](#dynamic-routing-and-code-splitting) | working |
| 9 | [Higher-Order Components](#higher-order-components) | working |
| 10 | [Render Props and Compound Components](#render-props-and-compound-components) | working |
...
**Misconception 1: React Testing Library tests implementation details - you should use Enzyme for that.**

React Testing Library (RTL) explicitly discourages testing implementation details - it encourages testing user behavior. RTL queries by role, label, and text that users see, not by component internal state, method names, or CSS class names. Enzyme, conversely, provides direct access to component internals (`shallow`, `instance()`, `state()`). RTL tests are more resilient to refactoring; Enzyme tests often break when implementation changes but behavior stays the same.

**Misconception 2: `waitFor` and `findBy` queries are interchangeable.**

`findBy*` queries return a promise that resolves when the element appears (they are `getBy` + automatic waitFor). `waitFor` is for arbitrary async assertions beyond element queries: `await waitFor(() => expect(mockFn).toHaveBeenCalled())`. Using `waitFor(getByRole(...))` is unnecessarily verbose; use `findByRole` instead. Using `findBy` for assertions other than element existence is the wrong choice; use `waitFor` for those.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: Test passes in isolation but fails in suite due to uncleared state.**

Symptom: test fails when run with other tests but passes when run alone; failure is non-deterministic. Root cause: global state (mocked module, context provider state, timer mocks) not cleaned up after test; previous test's state bleeds into current test. Diagnosis: run tests in isolation vs full suite; add cleanup logging. Fix: add proper cleanup in `afterEach`: `jest.clearAllMocks()`, `cleanup()` (automatic in RTL), restore timers: `jest.useRealTimers()`.

**Failure Mode 2: Querying by test ID creates brittle tests.**

Symptom: tests break when UI is redesigned even though the behavior is identical; tests require maintaining parallel `data-testid` attributes in production code. Root cause: `getByTestId` used as primary query method. Fix: prefer accessible queries in order: `getByRole` > `getByLabelText` > `getByPlaceholderText` > `getByText` > `getByTestId`; use `data-testid` only when no semantic query is possible.
...
**Misconception 1: Deeply nested component hierarchies are a sign of good componentization.**

Deep nesting indicates over-componentization or prop drilling problems. A stack of `<PageLayout> > <ContentArea> > <MainSection> > <ArticleContainer> > <ArticleBody>` where each component only wraps a single child is indirection without benefit. Aim for flat component hierarchies: components should have meaningful responsibilities, not just provide structural wrappers.

**Misconception 2: Deriving state in useEffect is the correct pattern for state synchronization.**

Deriving state in useEffect creates a render lag: the component renders with old derived state, then the effect fires, updates state, causing another render. If state B should always equal a transform of state A, compute B from A directly in the render (no useEffect needed). If an external event should update state, handle it in the event handler. `useEffect` for state synchronization is an anti-pattern that causes extra renders and potential infinite loops.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: Index as key causes incorrect state preservation on list reorder.**

Symptom: reordering a list causes components to retain state from their previous positions; animated transitions flash incorrectly; input values appear in the wrong list items after sort. Root cause: `key={index}` - React maps component identity to array position; when the array reorders, React thinks the same components are still in the same positions and updates their props without remounting. Fix: use stable unique IDs as keys: `key={item.id}`.

**Failure Mode 2: useEffect with missing dependencies creates silent bugs that are hard to reproduce.**

Symptom: component occasionally shows stale data; behavior depends on the order of renders and state updates in ways that are hard to reproduce consistently. Root cause: ESLint exhaustive-deps rule is disabled or ignored; effects use values not in their dependency arrays. Fix: enable and obey the `react-hooks/exhaustive-deps` ESLint rule; never suppress the warning without understanding why - it exists to prevent this exact class of bug.
