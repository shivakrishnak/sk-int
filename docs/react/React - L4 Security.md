...
| # | Keyword | Weight |
|---|---|---|
| 1 | [React Security Vulnerabilities and Mitigations](#react-security-vulnerabilities-and-mitigations) | expert |
| 2 | [React Server Components and Suspense](#react-server-components-and-suspense) | expert |
| 3 | [React Fiber Reconciler Internals](#react-fiber-reconciler-internals) | expert |
| 4 | [React Testing Library Patterns](#react-testing-library-patterns) | intermediate |
| 5 | [React Anti-patterns](#react-anti-patterns) | intermediate |
| 6 | [Controlled vs Uncontrolled Components](#controlled-vs-uncontrolled-components) | intermediate |
| 7 | [React Query and Server-State Management](#react-query-and-server-state-management) | intermediate |
| 8 | [React Performance Optimization Techniques](#react-performance-optimization-techniques) | intermediate |
| 9 | [React.memo and Re-render Prevention](#reactmemo-and-re-render-prevention) | intermediate |
| 10 | [React Router and Client-side Routing](#react-router-and-client-side-routing) | working |
| 11 | [Dynamic Routing and Code Splitting](#dynamic-routing-and-code-splitting) | working |
| 12 | [Higher-Order Components](#higher-order-components) | working |
| 13 | [Render Props and Compound Components](#render-props-and-compound-components) | working |
...
**Misconception 1: JSX auto-escaping makes React applications immune to XSS.**

JSX auto-escapes string values rendered as text content, preventing most XSS. However, auto-escaping does NOT protect: `dangerouslySetInnerHTML` (explicit raw HTML injection), `javascript:` URLs in `href` attributes (`<a href={userUrl}>`), event handler injection via object spread (`<Component {...userProps}>`), and postMessage-based attacks where message content is used in rendering without sanitization. These vectors require explicit developer attention.

**Misconception 2: CORS headers on the API prevent all Cross-Site Request Forgery attacks.**

CORS prevents cross-origin READS (a malicious site cannot read your API response) but does NOT prevent CSRF (a malicious site can make cross-origin STATE-CHANGING requests that a browser will send with the user's cookies). CORS and CSRF are different attack vectors requiring different mitigations. CSRF protection requires: SameSite cookie attribute, CSRF tokens in mutation requests, or requiring a custom header (e.g., `X-Requested-With`) that simple cross-origin forms cannot set.

---

### 🚨 Failure Modes and Diagnosis

**Failure Mode 1: dangerouslySetInnerHTML used with unsanitized user content creates persistent XSS.**

Symptom: injected `<script>` tags or `<img onerror>` payloads from user content execute in other users' browsers; attacker can steal session tokens, perform actions as the victim. Diagnosis: search for all `dangerouslySetInnerHTML` usages; verify each source is either static trusted content or sanitized. Fix: sanitize with DOMPurify before rendering: `dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(content) }}`; add Content Security Policy headers to limit script execution.

**Failure Mode 2: Sensitive data exposed in client bundle via over-fetching or logging.**

Symptom: Redux DevTools or network tab reveals full user objects including private fields (password hashes, internal IDs, payment details); sensitive API responses cached in the application. Root cause: API returns entire database objects; client-side store holds full server response including fields that should not be client-side. Fix: implement API response filtering (return only fields the client needs); avoid logging full state objects in production; use Redux DevTools with sanitized state snapshots.
