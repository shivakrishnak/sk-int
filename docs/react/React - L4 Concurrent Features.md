...
SSR renders components to HTML on the server but sends the full JavaScript bundl

**Misconception 2: Suspense is only for code splitting with React.lazy.**

Suspense works with any asynchronous resource that implements the suspend protoc
...
Symptom: build error "Server Component cannot be imported in a Client Component"

**Failure Mode 2: Suspense boundary placed too high causes entire page to show loading state.**

Symptom: whole page shows spinner while a single data-dependent section loads; u
